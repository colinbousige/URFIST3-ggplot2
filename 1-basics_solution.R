...
mtcars |> # we work on the mtcars dataset, send it to ggplot
    ggplot(aes(y = factor(cyl), group=cyl))+ # define the x and y variables of the plot, and also the color
        geom_bar(aes(fill = after_stat(count)),
                 stat="count", orientation = "y") +   # plot with bars
        geom_text(aes(x=after_stat(count-1),
                      label=after_stat(count)),
                  stat = "count", orientation = "y", size=14, color='white') + # add the count as a label
